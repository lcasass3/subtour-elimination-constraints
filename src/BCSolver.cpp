...
{
    if (!context.inCandidate() && !context.inRelaxation())
        return;
    ...
    IloNumArray Var(env);
    if (context.inRelaxation())
        context.getRelaxationPoint(_x, Var);
    else
        context.getCandidatePoint(_x, Var);
    ...
    auto it = max_element(_edges->begin(), _edges->end(), [&Var](const edge &e1, const edge &e2)
                          { return Var[e1.id] < Var[e2.id] || (Var[e1.id] == Var[e2.id] && e1.w < e2.w); });
    tour.push_back(it->i);
    tour.push_back(it->j);
    n -= 2;
    ...
    while (n != 0)
    {
        ...
        if ((*_edges)[bestedgeid].i == i)
            tour.insert(tour.begin(), (*_edges)[bestedgeid].j);
        else if ((*_edges)[bestedgeid].j == i)
            tour.insert(tour.begin(), (*_edges)[bestedgeid].i);
        else if ((*_edges)[bestedgeid].i == j)
            tour.push_back((*_edges)[bestedgeid].j);
        else
            tour.push_back((*_edges)[bestedgeid].i);
        n--;
    }
    ...
    if (incumbent > length)
    {
        ...
        for (int i = 0; i < tour.size(); i++)
        {
            ...
            if (it != _edges->end())
                isEdgeInTour[it->id] = true;
            else
                throw exception("Edge not found (Heuristic)");
        }
